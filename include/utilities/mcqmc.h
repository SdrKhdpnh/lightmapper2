...
typedef unsigned int Fnv32_t; //why is "unsigned long" still used for 32bit u_int??
#define FNV1_32_INIT ((Fnv32_t)0x811c9dc5)
#define FNV_32_PRIME ((Fnv32_t)0x01000193)

inline Fnv32_t fnv_32a_buf(Fnv32_t value)
{
	Fnv32_t hash=FNV1_32_INIT;
	// first octet
	hash ^= value & 0x000000ff;
	hash *= FNV_32_PRIME;
	// second octet
	value >>= 8;
	hash ^= value & 0x000000ff;
	...
	hash ^= value & 0x000000ff;
	hash *= FNV_32_PRIME;
	// c'mon, you should get it by now...
	value >>= 8;
	hash ^= value & 0x000000ff;
	hash *= FNV_32_PRIME;
