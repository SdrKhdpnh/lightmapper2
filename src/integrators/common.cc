			...
			for(int i=0; i<n; ++i)
			{
				...
				if( (*l)->illumSample (sp, ls, lightRay) )
				{
					...
					if(!shadowed)
					{
						...
						color_t surfCol = material->eval(state, sp, wo, lightRay.dir, BSDF_ALL);
						//lightPdf *= std::fabs(sp.N*lightRay.dir);
						if( lightPdf>1.f && (*l)->canIntersect() ) // bound samples and compensate by sampling from BSDF later
						{
							bias=true;
							lightPdf = 1.f;
						}
						ccol += surfCol * ls.col * std::fabs(sp.N*lightRay.dir) * lightPdf;
					}
				}
			}
...
//	BSDF_t bsdfs;
	...
	for(std::vector<light_t *>::const_iterator l=lights.begin(); l!=lights.end(); ++l)
	{
		...
		if( (*l)->diracLight() )
		{
			...
			{
				// ...shadowed...
				lightRay.tmin = 0.0005f; // < better add some _smart_ self-bias value...this is bad.
				shadowed = (trShad) ? scene->isShadowed(state, lightRay, sDepth, scol) : scene->isShadowed(state, lightRay);
				if(!shadowed)
				{
					if(trShad) lcol *= scol;
        ...
        else if ((*l)->ambientLight()) // Bioware specific
        {
            // We should really get a ray from the surface and sample the material. For now, the material
            // is manually created white, so we'll just return the colour.
			lSample_t ls;
			if( (*l)->illumSample (sp, ls, lightRay) )
			{
                col += ls.col;
...
{
	float r_p=sqrt(r_photon2), r_g=sqrt(r_gather2);
