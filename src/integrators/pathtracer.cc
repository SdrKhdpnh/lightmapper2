...
bool pathIntegrator_t::preprocess()
{
	...
	else if(causticType == BOTH)
	{
		...
		for(li=sceneLights.begin(); li!=sceneLights.end(); ++li)
		{
	...
	if(scene->intersect(ray, sp))
	{
		...
		if(bsdfs & path_flags)
		{
			...
			for(int i=0; i<nPaths; ++i)
			{
				...
				if(s.pdf > 1.0e-6f) scol *= (std::abs(pRay.dir*sp.N)/s.pdf);
				else continue;
				throughput = scol;
				...
				for(int depth=1; depth<bounces; ++depth)
				{
					...
					if(s.pdf > 1.0e-6f) scol *= (std::abs(pRay.dir*hit->N)/s.pdf);
					else break;
