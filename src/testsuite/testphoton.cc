...
//	params["type"] = parameter_t(std::string("pathtracing"));
    ...
    // dant: We need to add a volume integrator too, or the whole thing will crash
    params.clear();
    params["type"] = parameter_t(std::string("EmissionIntegrator"));
    volumeIntegrator_t* pVolIntegrator = (volumeIntegrator_t*)env->createIntegrator("myVolIntegrator",params);
    scene->setVolIntegrator(pVolIntegrator);
